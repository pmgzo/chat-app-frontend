...
export default function FriendSuggestions() {
	...
	const { data: unrespondedFriendRequestsData } = useSuspenseQuery(GET_UNRESPONDED_FRIEND_REQUESTS);
	...
	return (
		<div className="w-60">
			...
			<ul className="grid grid-cols-1 gap-4 w-full overflow-y-scroll">
				...
				.concat(unrespondedFriendRequestsData.myUnrespondedFriendRequests.map(({
					id: friendshipId,
					peer: {id: peerId, name}
				}: {
					id: number,
					...
					<li key={`unrespondedFR-${friendshipId}-${peerId}`} className="w-full bg-gray-200 h-[5rem] rounded-xl">
						<div className="flex flex-col justify-center h-full">
							<div className="flex justify-center text-black text-center">
								{name}
							</div>
							<div className="flex justify-center">
								<div className="text-[12px] w-fit bg-gray-400 rounded mt-2 px-1 text-center">
									Sent
								</div>
							</div>
						</div>
